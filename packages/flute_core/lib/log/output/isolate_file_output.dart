  ...
  Future<void> _initIsolate() async {
    try {
      ...
      _isolate = await Isolate.spawn(
        _isolateMain,
        _IsolateInitData(_receivePort.sendPort, config, rootIsolateToken),
      ...
      _receivePort.listen((message) {
        if (message is SendPort) {
          ...
          _initCompleter.completeError(
            message[1],
            StackTrace.fromString(message[2]),
          );
        }
      });
    } catch (e, s) {
...
class _IsolateWorker {
  ...
  Future<void> flush() async {
    ...
    try {
      ...
      final lines = events
          .expand((e) => e.lines.map(_removeAnsiCodes))
          .join('\n');
...
class _IsolateInitData {
  ...
  _IsolateInitData(this.mainSendPort, this.config, this.rootIsolateToken);
}
